            ...
            <div class="bg-white p-4 rounded shadow">
                ...
                @forelse ($order->pizzas as $pizza)
                    <div class="mb-4 border-b pb-2">
                        ...
                        $baseToppings = $toppings->filter(fn($t) => $t->pivot->is_extra === false)->pluck('name')->toArray();
                        $extraToppings = $toppings->filter(fn($t) => $t->pivot->is_extra === true)->pluck('name')->toArray();

                        @endphp

                        @if (!empty($baseToppings))
                            <p>Base Toppings: {{ implode(', ', $baseToppings) }}</p>
                        @endif

                        @if (!empty($extraToppings))
                            <p>Extra Toppings (85p): {{ implode(', ', $extraToppings) }}</p>
                        @endif
                    </div>
                @empty
